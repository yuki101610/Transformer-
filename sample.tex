...
\begin{frame}{「状態更新の追跡」に関する研究}
    振る舞い：物体や人物の状態(位置・所有権)が次々と変わる時, それらを正確に追跡する\\
    \vspace{0.5em}
    仮説：
    \begin{enumerate}\scriptsize
        \item 状態を上書きする(過去情報を抑制, 新しい情報を活性化させる)回路が存在する
        \begin{itemize}\scriptsize
            \item 「状態A：鍵が箱に入っている。, 状態B：鍵が箱から棚へ移される。, 質問：鍵は現在どこ？, 答え：棚」この時, Aを抑制, Bを活性化, または抑制+活性化させる回路が存在する
        \end{itemize}\scriptsize
        \item 古い情報と新しい情報から正解を読み出す回路が存在する
        \begin{itemize}\scriptsize
            \item A,BからBを選ぶ読み出し回路が存在する
        \end{itemize}\scriptsize
    \end{enumerate}
\end{frame}
